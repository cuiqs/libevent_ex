...
struct user_data
{
	char *name;/* the name we are resolving*/
	int idx; /*It`s position on the command line */
};
...
int main(int argc,char *argv[])
{
	...
	//	hints.ai_flags=EVUTIL_AI_CANONNAME;
		...
		if(!(u_data=malloc(sizeof(struct user_data))))
		{
			perror("malloc");
			exit(1);
		}
		if(!(u_data->name=strdup(argv[i])))
		{
			perror("strdup");
			exit(1);
		}
		...
		++n_pending_requests;
		req=evdns_getaddrinfo(dnsbase,argv[i],NULL,&hints,callback,u_data);
		if(req==NULL)
